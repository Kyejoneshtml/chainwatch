...
**Status correctness never means `FINAL`.** That was the design error this document originally shipped with, and it contradicted itself: `checkpoints` (below) was already specified with `ORDER BY last_run_at DESC LIMIT 1 BY component` for exactly this reason, while this section told every other status read to use `FINAL` instead. A query that already filters to one known key (`WHERE txid = '{txid}'`, `WHERE component = '{component}'`) gets deduplication at read time more cheaply the same way: `ORDER BY <version column> DESC LIMIT 1` (or `LIMIT 1 BY <key>` when the query covers several keys at once, as `checkpoints` does). Measured live on `transactions` (10M+ rows): both approaches read the same number of rows for a single known `txid` — the primary index already prunes to that key's own unmerged versions regardless of `FINAL` — but `FINAL` reads roughly double the bytes (needs every column to run the merge comparison) and takes roughly twice as long, for the identical result. Reserve `FINAL` for the one place it is actually the right tool: verifying that deduplication itself is working (the continuous verification query below), where the whole point is testing whether `FINAL` correctly collapses every duplicate — replacing it there with `LIMIT 1 BY` would make the check vacuously pass regardless of whether anything was actually broken.
