...
```
**Correction, found by the production detector diff below, not by re-reading the code more carefully: this is not a correctness bug in rule 1 or rule 2, and the claim that it was is wrong.** The reasoning at the time was that `common.find_input_side_candidates` picks the value from whichever copy has the *earliest* `seen_at`, and for these 44,502 inputs that's the placeholder (`value=0`) — but that reasoning skipped over the function's own `WHERE address = '{address}'` clause. The placeholder row's `address` is `''`; the caller always passes a real, validated address string; `'' = 'bc1q...'` can never match. The placeholder is excluded by the query itself, before the Python "earliest wins" logic ever sees it — confirmed directly against this exact example: querying `flows_old` with the address filter returns only the resolved row (`value=16627`), never the placeholder. The trap is real as a storage-level duplication defect (two rows for one input, one of them wrong) — it just never reached either rule's actual output, because the address equality both rules already filter on happened to be the same thing that would have needed to change for the placeholder to leak through.
...
**Production migration: complete, 2026-09-24.** Preconditions checked first: no ingestor or detector process running against production, `flows` unchanged at 67,144,753 rows since the redesign was built.

**Step 0, baseline and accounting.** Every one of `flows`'s 25,764,935 distinct `(txid, direction, position)` keys categorised — `direction='out'` (15,060,660, always kept), `direction='in'` with a resolved copy (10,686,812, kept), `direction='in'` with no resolved copy but a confirm-path copy i.e. `block_height != 0` (0 — the pruned-undo-window terminal-unresolved case is real but has never actually occurred in this project's history), `direction='in'` with neither (17,463, excluded — mempool-only `parent_pending`/`unresolved`). Checked against the live write path before excluding anything: `persist.build_flow_rows`'s filter skips both `parent_pending` and `unresolved` identically when called from the mempool path, so the live code would never have written these rows either — excluding them matches live behaviour, not a data-loss risk. Kept (25,747,472) + excluded (17,463) = 25,764,935, matching the independently-computed total exactly.

**Step 1–2, `flows_new` created and backfilled.** `LIMIT 1 BY txid, direction, position` (ordered by resolved-first, then latest `seen_at`) tested against `row_number()` on the same bucket first: 431.03 MiB / 66,834,566 rows read vs. 631.40 MiB / 67,144,753 rows read for an identical result — `LIMIT 1 BY` used. First backfill attempt at 32 hash-buckets failed cleanly on every single chunk (the real `INSERT` needs more memory than a `SELECT`-only dry run, from write buffering and compression overhead) — `flows_new` stayed at 0 rows throughout, nothing lost. Re-run at 128 buckets: all succeeded, zero errors, final count **25,747,472 — an exact match** against Step 0's target. Spot-checked both previously-found cases directly: the sixfold-replayed key collapsed to exactly one row; the parent-pending trap key shows the real resolved value (`16627`, not the placeholder `0`). Zero duplicate keys anywhere in `flows_new`, checked exhaustively (hash-bucket partitioned).

**The materialized view: tested on regtest before touching production, and the test changed the plan.** The working assumption was that a view attached to `flows` would either keep following the renamed-away `flows_old`, or fail loudly if it didn't. Neither is quite right. **A materialized view resolves its source table by name at each push, not by a fixed reference captured when it was created.** Tested directly: rebuilt production's exact current state on regtest (old-shaped `flows`, `address_stats_mv` still referencing `block_time`, matching production exactly — `schema/09`'s fix was never applied there, confirmed separately by reading production's live `SHOW CREATE TABLE`), then ran the real rename. Inserting into the newly-renamed `flows` (the redesigned table) triggered the *old* MV's query, which failed on the now-missing `block_time` — while the base table insert itself still landed underneath (1 row, confirmed). Inserting into `flows_old` afterward did not trigger the MV at all. So the MV follows whatever is currently named `flows`, never `flows_old` — meaning a rename with the view left as-is would have reproduced the exact flush-failure-forever shape already found once this session, this time against real production traffic, the moment the ingestor started writing again.

**Decision: drop `address_stats_mv` rather than fix it.** `address_stats` is already inflated by the ~41.38 million duplicate rows measured before the redesign; reattaching a working view would add correct increments on top of already-wrong totals, indefinitely, never converging back to correct. Dropping the view freezes `address_stats` as a stale snapshot instead — deliberately stale, not deliberately wrong. Guarded before running: confirmed the view was created with an explicit `TO address_stats` target, not an implicit inner storage table (`has_select=1`, no `.inner%` table exists anywhere) — dropping it cannot delete `address_stats`'s own data. Command run: `DROP TABLE IF EXISTS address_stats_mv`. Verified after: `address_stats` intact at 10,244,591 rows, `flows`'s `dependencies_table` empty. `schema/10_drop_address_stats_mv.sql` records this so a fresh bootstrap reaches the same end state production now has. `address_stats` itself is not dropped — a table still there for whichever of the two options recorded above (query `flows FINAL` at read time, or rebuild from a deduplicated source) eventually replaces the view.

`ingestor/schema_check.py` extended with a fourth guard: refuses to start if anything depends on `flows` that isn't explicitly expected (`EXPECTED_FLOWS_DEPENDENTS`, now empty, since the one MV that ever existed is dropped, not fixed). Verified both branches live against production: a throwaway test MV attached to `flows` was correctly caught and refused (`unexpected object(s) depending on flows: ['zzz_test_mv']`), then cleaned up; with nothing attached, the check passes.

**Step 4, the rename.** `RENAME TABLE flows TO flows_old, flows_new TO flows` — atomic. Verified immediately: `flows` is `ReplacingMergeTree`, 25,747,472 rows; `flows_old` is `MergeTree`, 67,144,753 rows, fully intact. `schema_check.verify_flows_schema` passes cleanly against the live post-rename table.

**Step 5, the detector dry-run comparison.** Rather than writing to `alerts` or advancing checkpoints, the real `detection/common.py`, `wallet_drain.py` and `fan_in_consolidation.py` functions were called directly — `find_input_side_candidates`, `find_receiving_txids`, `address_totals`, `sources_within_window` — against the live post-rename `flows`, compared against textually-identical queries run against `flows_old` (no further renames needed; the only difference is which table name the query text targets). Three addresses with substantial real history: the three test addresses previously in `watchlist` (`active=0` now, but their historical data is exactly what a fair before/after comparison needs). Result: **zero differences, across every metric, for all three** — not even the one difference expected going in. That result was checked rather than accepted at face value, since it directly contradicted this document's own earlier claim (corrected above) that the parent-pending trap was a live correctness bug in rule 1/2's candidate values: it re-confirmed the correction directly, on the exact concrete example already cited — `find_input_side_candidates`, called against `flows_old` for `bc1q2kv27d25vmfku6n8zd62hwetwhh5w72kxpw8cd`, returns only the resolved row (`value=16627`) even before the migration, because its own `address = '{address}'` filter already excludes the placeholder. The migration changes what is stored; it was never going to change what these three functions returned, because they were already immune, the same way they were already immune to replay. Not an unexplained difference, so not reverted.

**Not done:** `flows_old` is not dropped (kept as the safety net this plan called for). `address_stats`'s own redesign is unchosen. Both explicitly separate, later decisions.
...
### The 49 wallet_drain alerts from the backlog-clearing restart are an observation-window artifact, not 49 detections

After the fixes above (silent truncation, `FINAL` point lookups, resumable catch-up) were deployed and the ~1,380-block mainnet backlog cleared, the fresh restart's first `wallet_drain` run wrote 49 alerts (`watches=22 candidates=461,907 alerts_written=49`). Checked before reading 49 as a meaningful number, per this project's standing discipline of verifying rather than accepting a measurement at face value: `alerts.detail`'s `observation_window_seconds` field (`build_alert`, `detection/wallet_drain.py`) for every one of the 49.

Result: **all 49, not most.** Every value falls between 13 and 418 seconds — the longest is under seven minutes, nowhere close to the one-day threshold that would distinguish a real short-window drain from an artifact.

The mechanism: `first_seen_for_address` is `min(seen_at)` over this project's own `flows` rows for the address — when *this ingestor* first observed it, not the address's real on-chain history (no `txindex`, no historical backfill, per `CLAUDE.md`). The watchlist's 22 addresses were only added shortly before the backlog-clearing restart, so their first `flows` rows were written during that restart's own burst — and `wallet_drain`'s "balance before" is `total_received - total_spent` summed over exactly that same short history. An address that held real, pre-existing balance before monitoring began looks, from inside this narrow window, exactly like a full drain: "every balance this project has observed... consumed in one transaction" (the alert's own `observation` text) is true by construction whenever total observed history is itself only seconds to minutes old, independent of whether anything unusual happened on-chain. This is the same holdings-visibility gap this document already names in `utxo_visibility_limitation` — measured directly here for the first time, rather than assumed.

**49 is not 49 detections.** It is 49 addresses whose first-ever observed spend happened to be evaluated while their observation window was still single-digit minutes wide — a predictable, mechanical consequence of adding 22 addresses and then immediately clearing a multi-day backlog in one burst, not a signal about drain activity. A meaningful measurement of rule 2's real false-positive population needs alerts whose `observation_window_seconds` is large relative to typical spend frequency — the multi-day soak period now running (all four processes left running, per instruction) is what will produce that. Any manual classification of `wallet_drain` shadow alerts (`Definition of done`, below) should filter or bucket by `observation_window_seconds` and treat short-window alerts as not yet informative, rather than folding them into the same 100-alert sample as genuine long-window observations.

Not fixed in code — this is a property of monitoring an address that already had a balance, not a bug. `known_txids` (`ingestor/confirm.py`), flagged above as a recurring `MEMORY_LIMIT_EXCEEDED` failure during that same backlog-clearing run, was removed rather than optimized: it fed only `stats.tx_confirmed_new`, a shutdown-summary-only stat with no consumer during a run meant to last days (shutdown may not happen for the whole observation period), redundant with the more rigorous catch-up message-loss investigation above, and not worth carrying a live failure mode for a number nobody was reading. `KNOWN_TXIDS_CHUNK`'s chunking was the other option (shrink it further rather than remove the call) but was rejected: it only lowers the odds of the same failure, it does not remove it, and the value it protects is not load-bearing enough to justify tuning it under load. Built and committed, **not yet live** — the running ingestor process (left running, per instruction) still has the old code in memory and will keep hitting this failure occasionally until it is next restarted; the periodic catch-up retry continues to absorb it exactly as it did during the backlog clear, so this is not urgent.
